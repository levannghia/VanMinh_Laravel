                                ...
                                <div class="border-col">
                                    <div class="detail-product-link">
                                        <a href="{{ route('get.product.slug', $item->slug) }}"><img
                                                src="public/upload/images/product/thumb/{{ $item->photo }}" alt="{{$item->name}}"
                                                width="200px"> </a>
                                    </div>
                    ...
                    @if ($key == 0)
                        @foreach ($category_noibac as $item)
                            <span class="tablinks{{ $key }}" category-id="{{ $item->id }}"
                                onclick="chemistry(event, 'chemistry-{{ $item->id }}')">{{ $item->name }}
                            </span>
                            {{ session(['id_cate1' => $item->id]) }}
                        @endforeach
                    @else
                        @foreach ($category_noibac as $item)
                            <span class="tablinks{{ $key }}" category-id="{{ $item->id }}"
                                onclick="medicalEquipment(event, 'medical-equipment-{{ $item->id }}')">{{ $item->name }}
                            </span>
                            {{ session(['id_cate2' => $item->id]) }}
                        @endforeach
                    @endif
                    ...
                    @foreach ($category_noibac as $item)
                        {{-- @php
                            $cate_product = DB::table('products')
                                ->select('products.id', 'products.name', 'products.price', 'products.view', 'products.photo', 'categories.name AS category_name')
                                ->join('categories', 'categories.id', '=', 'products.category_id')
                                ->where('categories.id', $item->id)
                                ->where('products.type', 0)
                                ->where('products.status', 1)
                                ->orderBy('categories.stt', 'ASC')
                                ->paginate($settings['PHAN_TRANG_PRODUCT']);
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                                    ...
                                    <g transform="matrix(1.43 0 0 1.43 100.39 95.98)" id="XDewt6JbuW4lwAwmXcCht">
                                        <image
                                            ...
                                            xlink:href="public/upload/images/standard/thumb/{{ $item->photo }}" x="-25" y="-25"
                ...
                @foreach ($video as $key => $item)
                    <div class="item">
                        <iframe style="width: 100%;" height="250"
                            src="https://www.youtube.com/embed/{{ $item->link_youtube }}" title="YouTube video player"
                            frameborder="0"
